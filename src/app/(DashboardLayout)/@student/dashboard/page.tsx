...
import {
  ...
  BookOpen, Plus, Menu, X, Loader2, Sparkles, Target, Save
} from "lucide-react";
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // আপডেট হওয়া নাম লোকাল স্টেটে সেট করা
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-8">
          {loading ? (
            ...
            <>
                     ...
                     <div className="space-y-8">
                        <div className="rounded-[2.5rem] bg-slate-900 p-8 text-white relative group overflow-hidden shadow-2xl">
                           ...
                           <div className="relative z-10">
                              ...
                              <p className="text-sm opacity-80 mt-2 font-medium">Keep going to reach your weekly learning target!</p>
              ...
              {active === "profile" && (
                <div className="max-w-4xl mx-auto rounded-[2.5rem] bg-card border border-border p-10 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <div className="space-y-6">
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div className="space-y-2">
                           ...
                           <input 
                              ...
                              className="h-14 w-full bg-muted/40 border border-border rounded-2xl flex items-center px-5 font-bold text-sm focus:ring-2 focus:ring-primary outline-none transition-all"
                           />
                        </div>
                        <div className="space-y-2">
                           <label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest ml-1">Email Address</label>
                           <div className="h-14 w-full bg-muted/10 border border-border rounded-2xl flex items-center px-5 font-bold text-sm text-muted-foreground cursor-not-allowed">
                              {user?.email}
                           </div>
                        </div>
                     </div>
                     <button 
                        ...
                        disabled={updateLoading || !newName || newName === user?.name}
                        className="flex items-center gap-2 px-8 h-14 bg-primary text-primary-foreground rounded-2xl font-black text-sm shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100"
                     >
