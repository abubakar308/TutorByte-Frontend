        ...
        <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <Link href="/" className="group flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="mt-0.5 text-[11px] text-muted-foreground">Learn · Book · Grow</p>
            </div>
          </Link>
            ...
            {!user ? (
              ...
              <div className="ml-1 flex items-center gap-2">
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  >
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary border border-primary/20 overflow-hidden">
                      {user.avatarUrl ? (
                  ...
                  {profileOpen && (
                    <div className="absolute right-0 top-[calc(100%+8px)] w-56 overflow-hidden rounded-2xl border border-border bg-card shadow-xl shadow-black/10 animate-in fade-in zoom-in-95 duration-200">
                      <div className="border-b border-border/60 px-4 py-3">
                        <p className="text-sm font-semibold text-card-foreground line-clamp-1">{user.name}</p>
                        <p className="mt-0.5 text-[10px] uppercase tracking-widest font-bold text-muted-foreground">{cfg?.label} Account</p>
                      </div>
            ...
            <div className="mx-auto flex max-w-7xl flex-col gap-1 px-4 py-3 sm:px-6">
              {navLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="rounded-xl px-4 py-2.5 text-sm font-medium text-foreground/80 transition hover:bg-muted"
              ...
              <div className="mt-2 border-t border-border pt-3">
                {!user ? (
                  <div className="flex gap-2">
                    <Link href="/login" className="flex-1 rounded-xl border border-border px-4 py-2.5 text-center text-sm font-medium">Sign in</Link>
                    <Link href="/register" className="flex-1 rounded-xl bg-primary px-4 py-2.5 text-center text-sm font-semibold text-primary-foreground">Get started</Link>
                  </div>
                ) : (
        ...
        <div className="fixed inset-0 z-[60] flex items-start justify-center px-4 pt-[15vh]">
          ...
          <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-card shadow-2xl animate-in fade-in slide-in-from-top-4">
            <div className="flex items-center gap-3 border-b border-border px-4 py-3.5">
              ...
              />
              <button onClick={() => setSearchOpen(false)} className="rounded-md border border-border px-2 py-1 text-[10px] text-muted-foreground uppercase font-bold">ESC</button>
            </div>
          </div>
        </div>
      )}
